...
export type SharpApiActions<Actions> = Actions extends Record<string, (...params: any) => void>
  ? {
      [K in keyof Actions]: Actions[K] extends (api: any, ...params: infer Params) => void
        ? (...params: Params) => void
        : never;
    }
  : never;
...
export type SharpActions<Initializer extends SharpInitializer<any, any>> = {
  [k: string]: (api: ActionApi<SharpValue<Initializer>>, ...params: any[]) => void;
};
...
export type EnsureSignal<T> = [GetRidOfUndef<T>] extends [never]
  ? T
  : GetRidOfUndef<T> extends Signal<infer V>
    ? Signal<V>
    : T;
// type A = EnsureSignal<undefined>;
